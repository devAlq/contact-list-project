...
- Define a `struct ContactList` that manages the contacts as a **linked list** with a pointer to the head of the list.

- Implement the following functions:
  - `add()`: Adds a new contact to the end of the list and prints a confirmation message.  
  - `delete()`: Deletes a contact by name. If the contact is found, it removes the contact and prints a confirmation message. If not found, it prints a message indicating that the contact does not exist.  
  - `update()`: Updates the phone number associated with a given name and prints a confirmation message. If the name is not found, it prints a message indicating that the contact does not exist.  
  - `allContacts()`: Displays all the contacts in the list. If the list is empty, it prints a message indicating that no contacts are available.
...
- [ ] Completely define and manage the linked list in `ContactList`.
